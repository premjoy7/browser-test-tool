...
export default class PlayerProgress extends lng.Component {
    ...
    setProgressLive(start, end, currentTime) {
        this._progress = (currentTime - start) / Math.max(end - start, 1);
        this.tag("CurrentTime").text = Player.formatTime(currentTime);
        this.tag("Duration").text = Player.formatTime(end);

        this.tag("Active").color = 0xFFFF0000;
    }
